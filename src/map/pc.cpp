...
void pc_bonus(map_session_data *sd,int32 type,int32 val)
{
	...
	switch(type){
		...
		case SP_ADD_ITEM_SPHEAL_RATE:
			...
			break;
		case SP_SMOKE_GEAR_PROG:
			if (sd->state.lr_flag != LR_FLAG_ARROW)
				sd->special_state.smoke_gear_prog = val;
			break;
		default:
...
int64 pc_readparam( const map_session_data* sd, int64 type )
{
	...
	switch(type) {
		...
		case SP_SMOKE_GEAR_PROG: val = sd->special_state.smoke_gear_prog; break;
		default:
...
// Emundus pc_setsmoke
// pc_setsmoke(sd, 1); = ativa
// pc_setsmoke(sd, 0); = desativa
void pc_setsmoke(map_session_data* sd, int32 flag)
{
	if (!sd)
		return;
	...
	if (flag) {
		// Ativa smoke
		pc_setoption(sd, sd->sc.option | OPTION_SMOKE);
	}
	else {
		// Desativa smoke
		if (pc_isSmokeGear(sd)) // usando macro que você definiu
			pc_setoption(sd, sd->sc.option & ~OPTION_SMOKE);
	}
}
...
static int32 pc_checkcombo(map_session_data *sd, item_data *data) {
	...
	for (const auto &item_combo : data->combos) {
		...
		combo_idx.clear();
		//ShowWarning("Combo detectado | smoke_status = %d\n", item_combo->smoke_status);
		em_study_combo_item(sd, data);
		// Depois de analisar todos os itens, aplica os efeitos:
		if (item_combo->smoke_status >= 1 && item_combo->smoke_status <= 4) {
			sd->special_state.smoke_gear_prog = item_combo->smoke_status;
			pc_setreg(sd, add_str("SmokeGearProg"), item_combo->smoke_status);

			pc_setsmoke(sd, 1);  // ativa a flag
			UpdateSmokeStatus(sd);  // Checkcombo
			//ShowWarning("Combo detectado | smoke_status = %d\n", item_combo->smoke_status);
		}
		success++;
	}
...
bool pc_equipitem(map_session_data *sd,int16 n,int32 req_pos,bool equipswitch)
{
	...
	UpdateSmokeStatus(sd); // Equip Item
	return true;
}
...
bool pc_unequipitem(map_session_data *sd, int32 n, int32 flag) {
	...
	UpdateSmokeStatus(sd); // Unequip Item
	return true;
}
