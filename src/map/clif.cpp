...
void clif_parse_item_reform_start( int32 fd, map_session_data* sd ){
	...
	// If triggered from item - editado por Emundus
	if (sd->itemid == sd->state.item_reform) {
		if (base->consumeItemSuccess) {
			if (pc_delitem(sd, sd->itemindex, 1, 0, 0, LOG_TYPE_REFORM) != 0) {
				return;
			}
		}
	}
