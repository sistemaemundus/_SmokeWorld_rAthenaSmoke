...
// ============================================
// BUILDIN_FUNC: getcombos - Retorna total de combos ativos - EMUNDUS
// ============================================
BUILDIN_FUNC(getcombos)
{
	TBL_PC* sd_target;

	// Tenta obter o jogador alvo (pode ser o próprio ou especificado por ID/nome)
	if (!script_charid2sd(2, sd_target)) {
		// Se não conseguiu obter o alvo, retorna 0
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}

	// 🔹 limpa combos antigos
	sd_target->em_active_combos.clear();

	// 🔹 aplica combos de todos os equipamentos
	for (int16 i = 0; i < EQI_MAX; i++) {
		int16 idx = sd_target->equip_index[i];
		if (idx < 0) continue;

		item_data* eq_item = sd_target->inventory_data[idx];
		if (!eq_item) continue;

		em_study_combo_item(sd_target, eq_item); // só aplica, não soma
	}

	// 🔹 total correto é o tamanho final do vetor
	int total = sd_target->em_active_combos.size();

	// 🔹 atualiza count de cada combo
	for (auto& combo : sd_target->em_active_combos)
		combo.count = total;

	// 🔹 envia total para o script
	script_pushint(st, total);
	return SCRIPT_CMD_SUCCESS;
}

// ============================================
// BUILDIN_FUNC: getcombosall - Retorna string com todos os bônus dos combos ativos - EMUNDUS
// ============================================
/*BUILDIN_FUNC(getcombosall)
{
	TBL_PC* sd_target;

	if (!script_charid2sd(2, sd_target)) {
		script_pushstr(st, const_cast<char*>("Sem combos para mostrar..."));
		return SCRIPT_CMD_SUCCESS;
	}
	...
	for (int16 i = 0; i < EQI_MAX; i++) {
		...
		em_study_combo_item(sd_target, eq_item); // aplica combos
	}
	...
	if (total == 0) {
		const std::string sem_combos = "Sem combos para mostrar...";
		char* c_result = (char*)aMalloc(sem_combos.size() + 1);
		safestrncpy(c_result, sem_combos.c_str(), sem_combos.size() + 1);
		script_pushstr(st, c_result); // ⚠️ não chamar aFree depois
		return SCRIPT_CMD_SUCCESS;
	}

	// Monta string com delimitador "|"
    std::string result;
    for (size_t i = 0; i < sd_target->em_active_combos.size(); i++) {
        if (i > 0) result += "|"; // delimitador
        result += sd_target->em_active_combos[i].script_text;
    }
	...
	// envia para script
	char* c_result = (char*)aMalloc(result.size() + 1);
	safestrncpy(c_result, result.c_str(), result.size() + 1);
	script_pushstr(st, c_result);
...
// <--- [zBuffer] List of unit control commands
	...
	// COMANDOS DE EMUNDUS - SMOKEWORLD
	BUILDIN_DEF (getcombos, "?"),
	//BUILDIN_DEF (getcombosall, "?"),
