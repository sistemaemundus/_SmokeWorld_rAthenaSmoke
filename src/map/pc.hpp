...
// Cada jogador terá sua própria lista de combos ativos
struct em_active_combo_t {
	uint32 combo_id;           // ← campo obrigatório: item_combo->id
	std::string script_text;   // opcional: para debug, exibição ou fallback
	int count;                 // total de combos ativos (igual em todos)
};
...
public:
	...
	std::vector<em_active_combo_t> em_active_combos; // Emundus struct map_session_data
...
// Emundus
#define pc_isSmokeGear(sd) ((sd)->sc.option & OPTION_SMOKE)
