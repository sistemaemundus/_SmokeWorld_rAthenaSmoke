...
uint64 ItemReformDatabase::parseBodyNode( const ryml::NodeRef& node ){
	...
	if( this->nodeExists( node, "BaseItems" ) ){
		for( const ryml::NodeRef& baseNode : node["BaseItems"] ){
			...
			// CONSOME ITEM NO SUCESSO? BY EMUNDUS
			if (this->nodeExists(baseNode, "ConsumeItemSuccess")) {
				bool consume;

				if (!this->asBool(baseNode, "ConsumeItemSuccess", consume)) {
					return 0;
				}

				base->consumeItemSuccess = consume;
			}
			else {
				if (!base_exists) {
					base->consumeItemSuccess = true; // DEFAULT BY EMUNDUS
				}
			}
		}
	}
...
uint64 ComboDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	for (const auto& itemsit : items_list) {
		...
		if (this->nodeExists(node, "Smoke")) {
			if (!this->asBool(node, "Smoke", smoke_flag)) {
				this->invalidWarning(node["Smoke"], "Invalid value for 'Smoke' (must be true/false).\n");
				smoke_flag = false;
			}
		}

		combo->smoke_status = smoke_flag ? 1 : 0;
		...
		if (this->nodeExists(node, "Script")) {
			...
			if (!this->asString(node, "Script", script))
				return 0;
			// salva a string legível - Emundus
			combo->script_text = script;
			if (exists) {
